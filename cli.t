...
my $count_only = qx{printf 'Ada: TODO one\nBob: TODO two\nAda: TODO one\n' | $^X $script --count-only --contains one --dedupe-actions -}; is $count_only, "actions: 1\n", 'count-only composes contains and dedupe';
my $count_window = qx{printf 'TODO one\nTODO two\n' | $^X $script --count-only --lines 2:2 -}; is $count_window, "actions: 1\n", 'count-only composes line window';
my $count_bad = system("$^X $script --count-only --markdown - </dev/null >/dev/null 2>/dev/null"); isnt $count_bad, 0, 'count-only and markdown are mutually exclusive';
my $count_json_bad = system("$^X $script --count-only --json - </dev/null >/dev/null 2>/dev/null"); isnt $count_json_bad, 0, 'count-only and json are mutually exclusive';
